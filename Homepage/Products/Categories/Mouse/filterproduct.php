...
if(isset($_POST['action'])){
    $sql="SELECT * FROM mouse WHERE stock_count>1";
  ...
  ";
    ...
    if (isset($_POST["color"])) {
        $cpu_filter = implode("','", $_POST["color"]);
        $sql .= "
   AND color IN('" . $cpu_filter . "')
  ";
    }

    if (isset($_POST["connection"])) {
        $graphics_filter = implode("','", $_POST["connection"]);
        $sql .= "
   AND connection_type IN('" . $graphics_filter . "')
  ";
    }
  ...
  ";
    ...
    if(pg_num_rows($result)>0)
    {
        while($row=pg_fetch_assoc($result))
        {
            ...
            <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Mouse/Displaypage/productdisplaypage.php?Product_id='.$row["product_id"].'">Buy Now</a>
            </figure>';
        }
    }
