...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Mouse </h1>
...
$query="SELECT DISTINCT brand FROM mouse ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Color</h1>

<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT color FROM mouse ORDER BY color;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['color']; ?>
      <input type="checkbox"  id="colorcheckbox" class="filter_all brand" value="<?php echo $row['color']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Connection Type</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT connection_type FROM mouse ORDER BY connection_type;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['connection_type']; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['connection_type']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM mouse ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM mouse ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Mouse/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
...
$(document).ready(function() {
 ...
 $(".brand").click(function(){
    ...
    var color = get_filter_text('colorcheckbox');
    var connection = get_filter_text('connection_typecheckbox');
