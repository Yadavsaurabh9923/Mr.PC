...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Monitors </h1>
...
$query="SELECT DISTINCT brand FROM monitors ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Refresh Rate</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT refresh_rate FROM monitors ORDER BY refresh_rate;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['refresh_rate']; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['refresh_rate']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Response Time</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT responsetime FROM monitors ORDER BY responsetime;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['responsetime']; ?>
      <input type="checkbox"  id="connection_response" class="filter_all brand" value="<?php echo $row['responsetime']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM monitors ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM monitors ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Monitors/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
