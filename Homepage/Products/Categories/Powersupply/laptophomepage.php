...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Power Supply </h1>
...
$query="SELECT DISTINCT brand FROM powersupply ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Wattage</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT wattage FROM powersupply ORDER BY wattage;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['wattage']; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['wattage']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT form_factor FROM powersupply ORDER BY form_factor;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <input type="checkbox"  id="connection_response" class="filter_all brand" value="<?php echo $row['form_factor']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM powersupply ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM powersupply ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Powersupply/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
