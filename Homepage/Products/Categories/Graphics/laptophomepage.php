...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Graphic Cards </h1>
...
$query="SELECT DISTINCT brand FROM graphics ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Graphics Size</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT graphics_size FROM graphics ORDER BY graphics_size;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['graphics_size'],' GB'; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['graphics_size']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Graphics Type</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT graphics_ramtype FROM graphics ORDER BY graphics_ramtype;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['graphics_ramtype']; ?>
      <input type="checkbox"  id="connection_response" class="filter_all brand" value="<?php echo $row['graphics_ramtype']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM graphics ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM graphics ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Graphics/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
