...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Motherboards </h1>
...
$query="SELECT DISTINCT brand FROM motherboards ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Ram Technology</h1>

<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT ram_technology FROM motherboards ORDER BY ram_technology;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['ram_technology']; ?>
      <input type="checkbox"  id="cpu_namecheckbox" class="filter_all brand" value="<?php echo $row['ram_technology']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Graphics Technology</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT graphics_technology FROM motherboards ORDER BY graphics_technology;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['graphics_technology']; ?>
      <input type="checkbox"  id="graphics_typecheckbox" class="filter_all brand" value="<?php echo $row['graphics_technology']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM motherboards ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM motherboards ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Motherboards/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
...
$(document).ready(function() {
 ...
 $(".brand").click(function(){
    ...
    var cpu = get_filter_text('cpu_namecheckbox');
    var graphics = get_filter_text('graphics_typecheckbox');
