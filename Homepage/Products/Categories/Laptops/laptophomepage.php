...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Laptops </h1>
...
$query="SELECT DISTINCT brand FROM laptops ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">CPU</h1>

<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT cpu_name FROM laptops ORDER BY cpu_name;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['cpu_name']; ?>
      <input type="checkbox"  id="cpu_namecheckbox" class="filter_all brand" value="<?php echo $row['cpu_name']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Graphics</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT graphics_type FROM laptops ORDER BY graphics_type;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['graphics_type']; ?>
      <input type="checkbox"  id="graphics_typecheckbox" class="filter_all brand" value="<?php echo $row['graphics_type']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM laptops ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM laptops ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Laptops/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
