...
if(isset($_POST['action'])){
    $sql="SELECT * FROM processor WHERE stock_count>1";
  ...
  ";
    ...
    if (isset($_POST["formfactor"])) {
        ...
        $sql .= "
   AND processor_speed IN('" . $graphics_filter . "')
  ";
    }
    ...
    if (isset($_POST["usage"])) {
        ...
        $sql .= "
   AND total_cores IN('" . $usage_filter . "')
  ";
    }
    ...
    if (isset($_POST["responsetime"])) {
        ...
        $sql .= "
   AND total_threads IN('" . $responsetime . "')
  ";
    }
    ...
    if(pg_num_rows($result)>0)
    {
        while($row=pg_fetch_assoc($result))
        {
            ...
            <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Processor/Displaypage/productdisplaypage.php?Product_id='.$row["product_id"].'">Buy Now</a>
            </figure>';
        }
    }
