...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for RAM </h1>
...
$query="SELECT DISTINCT brand FROM ram ORDER BY brand;";
...
$query="SELECT DISTINCT ram_size FROM ram ORDER BY ram_size;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['ram_size'],' GB'; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['ram_size']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Voltage</h1>

<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT voltage FROM ram ORDER BY voltage;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['voltage'],' Volts'; ?>
      <input type="checkbox"  id="subcategorycheckbox" class="filter_all brand" value="<?php echo $row['voltage']; ?>">
      <span class="checkmark"></span>
      </label>
...
<?php
  ...
  $query="SELECT * FROM ram ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Ram/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
