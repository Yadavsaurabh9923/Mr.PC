...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for SSD </h1>
...
$query="SELECT DISTINCT brand FROM ssd ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Size</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT size FROM ssd ORDER BY size;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['size']; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['size']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">Hardware Interface</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT hardware_interface FROM ssd ORDER BY hardware_interface;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['hardware_interface']; ?>
      <input type="checkbox"  id="connection_response" class="filter_all brand" value="<?php echo $row['hardware_interface']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM ssd ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM ssd ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/SSD/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
