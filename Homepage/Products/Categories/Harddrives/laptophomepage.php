...
<br><br><br><br>
<h1 style="font-size: 2.1rem; padding-left: 20px; color: white;">Top Picks / Showing Results for Hard drives </h1>
...
$query="SELECT DISTINCT brand FROM harddrives ORDER BY brand;";
...
<h1 style="font-size: 1.5rem; color: white;">Drive Size</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT drive_size FROM harddrives ORDER BY drive_size;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['drive_size']; ?>
      <input type="checkbox"  id="connection_typecheckbox" class="filter_all brand" value="<?php echo $row['drive_size']; ?>">
      <span class="checkmark"></span>
      </label>
...
<h1 style="font-size: 1.5rem; color: white;">USB Interface</h1>
<?php

include "connection_db.inc.php";

$query="SELECT DISTINCT usb_interface FROM harddrives ORDER BY usb_interface;";
$result = pg_query($conn,$query);
...
if(pg_num_rows($result)>0)
  {
    ...
    {?>
      ...
      <label class="container"><?php echo $row['usb_interface']; ?>
      <input type="checkbox"  id="connection_response" class="filter_all brand" value="<?php echo $row['usb_interface']; ?>">
      <span class="checkmark"></span>
      </label>
...
$query="SELECT DISTINCT subcategory FROM harddrives ORDER BY subcategory;";
$result = pg_query($conn,$query);
...
<?php
  ...
  $query="SELECT * FROM harddrives ORDER BY product_id ASC;";
  $result = pg_query($conn,$query);
  $datas= array();
...
?>
    ...
    <a class="button" href="http://localhost/Website/Homepage/Products/Categories/Harddrives/Displaypage/productdisplaypage.php?Product_id=<?php echo $datas[$x]['product_id']?>">Buy Now</a>
    </figure>
